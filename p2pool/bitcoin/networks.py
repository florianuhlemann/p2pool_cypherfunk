...
nets = dict(
    ...
    cypherfunk=math.Object(
        P2P_PREFIX='fbc0b6db'.decode('hex'),
        P2P_PORT=33666,
        ADDRESS_VERSION=28,
        RPC_PORT=9999,
        RPC_CHECK=defer.inlineCallbacks(lambda bitcoind: defer.returnValue(
            'Cypherfunkaddress' in (yield bitcoind.rpc_help()) and
            not (yield bitcoind.rpc_getinfo())['testnet']
        )),
        SUBSIDY_FUNC=lambda height: 100000*100000000 >> (height - 14)//262800,
        POW_FUNC=lambda data: pack.IntType(256).unpack(__import__('ltc_scrypt').getPoWHash(data)),
        BLOCK_PERIOD=120, # s
        SYMBOL='FUNK',
        CONF_FILE_FUNC=lambda: os.path.join(os.path.join(os.environ['APPDATA'], 'Cypherfunk') if platform.system() == 'Windows' else os.path.expanduser('~/Library/Application Support/Cypherfunk/') if platform.system() == 'Darwin' else os.path.expanduser('~/.Cypherfunk'), 'Cypherfunk.conf'),
        BLOCK_EXPLORER_URL_PREFIX='http://cypherfunks.tuxhosting.de/block/',
        ADDRESS_EXPLORER_URL_PREFIX='http://cypherfunks.tuxhosting.de/address/',
        TX_EXPLORER_URL_PREFIX='http://cypherfunks.tuxhosting.de/tx/',
