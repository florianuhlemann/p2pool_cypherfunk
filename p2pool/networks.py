...
nets = dict(
    ...
    cypherfunk=math.Object(
        PARENT=networks.nets['cypherfunk'],
        SHARE_PERIOD=16, # seconds
        CHAIN_LENGTH=4*60*60//16, # shares
        REAL_CHAIN_LENGTH=4*60*60//16, # shares
        TARGET_LOOKBEHIND=32, # shares
        SPREAD=64, # blocks
        IDENTIFIER='e036c4df595addc4'.decode('hex'),
        PREFIX='7208b85e4da4cc6e'.decode('hex'),
        P2P_PORT=36663,
        MIN_TARGET=0,
        MAX_TARGET=2**256//2**20 - 1,
        PERSIST=True,
        WORKER_PORT=36363,
        BOOTSTRAP_ADDRS='94.23.196.92'.split(' '),
        ANNOUNCE_CHANNEL='',
